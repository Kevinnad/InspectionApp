...
public class TruckLoadingViewModel extends ViewModel {
    ...
    public void addData(TruckLoadingDataModel dataModel) {
        ...
        Completable.fromAction(new Action() {
            ...
            public void run() throws Throwable {
                Long key = dataBaseProvider.getAppDatabase().truckLoadingDao().insert(dataModel);
    ...
    public void getPreviousData() {
        ...
        if (currentId > 1) {
            dataBaseProvider.getAppDatabase().truckLoadingDao().getSingleInspection(currentId - 1).subscribeOn(Schedulers.io()).subscribe(new Consumer<TruckLoadingDataModel>() {
                @Override
   ...
   public void getAllItemData(String itemCode) {
    ...
    public void getLastInspection() {
        ...
        dataBaseProvider.getAppDatabase().truckLoadingDao().getLastInspection().subscribeOn(Schedulers.io()).subscribe(new Consumer<TruckLoadingDataModel>() {
            @Override
            public void accept(@NonNull TruckLoadingDataModel inspectionDataModels) throws Exception {
    ...
    private void handleResponse(TruckLoadingDataModel inspectionDataModels) {
